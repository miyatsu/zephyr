				...
				u16_t pkt_id, enum mqtt_packet type)
{
	...
	printk("MSG recived. type: %s, topic: ", str);
	for (int i = 0; i < msg->topic_len; ++i)
	{
		printk("%c", msg->topic[i]);
	}
	printk(" , msg: ");
...
static void app_mqtt_init(void)
{
	...
	for (i = 0; i < CONN_TRIES; i++) {
		rc = mqtt_connect(&client_ctx.mqtt_ctx);	/* TCP connection */
...
connected:
	...
	rc = try_to_connect(&client_ctx);		/* MQTT connection */
	PRINT_RESULT("try_to_connect", rc);
	if (rc != 0) {
		goto exit_app;
	}

	const char *topics[1] = {MQTT_SUBSCRIBEID};
	enum mqtt_qos topics_qos[1] = {MQTT_QoS1};
	rc = mqtt_tx_subscribe(&client_ctx.mqtt_ctx, sys_rand32_get(), 1, topics, topics_qos);
	k_sleep(APP_SLEEP_MSECS);
	PRINT_RESULT("subscribe \"dev/toRtos\"...\n", rc);
